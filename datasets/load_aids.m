function [ graphs_train, clss_train, graphs_valid, clss_valid,...
    graphs_test, clss_test ] = load_aids(folder)
    
    %% Load GREC database

    [fns_train,clss_train] = read_aids_cxl(fullfile(folder, 'train.cxl')) ;
    [fns_valid,clss_valid] = read_aids_cxl(fullfile(folder, 'valid.cxl')) ;
    [fns_test,clss_test] = read_aids_cxl(fullfile(folder, 'test.cxl')) ;
    ...
    for i = 1:ntrain
        [graphs_train(i).v, graphs_train(i).nl.values, graphs_train(i).e,...
            graphs_train(i).el.values] = read_aids_gxl(fullfile(folder, fns_train{i})) ;
...
%         graphs_train(i).am = double(graphs_train(i).am |graphs_train(i).am');
    ...
    for i = 1:nvalid
        [graphs_valid(i).v, graphs_valid(i).nl.values, graphs_valid(i).e,...
            graphs_valid(i).el.values] = read_aids_gxl(fullfile(folder, fns_valid{i})) ;
...
%         graphs_valid(i).am = double(graphs_valid(i).am |graphs_valid(i).am');
    ...
    for i = 1:ntest
        [graphs_test(i).v, graphs_test(i).nl.values, graphs_test(i).e,...
            graphs_test(i).el.values] = read_aids_gxl(fullfile(folder, fns_test{i})) ;
